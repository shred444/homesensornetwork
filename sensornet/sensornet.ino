...
void loop(void)
{
  ...
  while ( network.available() )
  {
    ...
    else if ( header.type == 'K' )
    {
      ...
      Serial.println("Parent Node: " + String(header.from_node));
  ...
  if ( this_node.address > 0 && temp_pin > -1 && ( ( Sleep && ! test_mode ) || send_timer.wasFired() ) && ! startup_leds )
  {
    ...
    message.temp_reading = dht.readTemperature(true);
    message.humidity_reading = dht.readHumidity();
    Serial.println("Temperature: " + String(message.temp_reading));
